...
int display(Mat im, CMT & cmt, int p, int nf)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        if (int(cmt.points_active.size())>0.5*p){
...
int main()
{
    ...
    Mat img1 = imread( "wood.jpg", IMREAD_GRAYSCALE );
    ...
    while (!bbox_flag && donot)
    {
        ...
        double cpu_percent = getCurrentValue();
        cout << "Percentage: " << cpu_percent << endl;
    ...
    while (true)
    {
        ...
        if (nf>180){
            ...
            while (!bbox_flag)
            {
                ...
                // Ptr<SIFT> detector = SIFT::create(0,3,0.01);
                Ptr<SIFT> detector = SIFT::create(0,3,0.02);
                std::vector<KeyPoint> keypoints1, keypoints2;
