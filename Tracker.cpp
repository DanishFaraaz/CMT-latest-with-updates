        ...
        vector<Point2f> & points_tracked, vector<unsigned char> & status)
{
    ...
    if (points_prev.size() > 0)
    {
        ...
        calcOpticalFlowPyrLK(im_prev, im_gray, points_prev, points_tracked, status, err, cv::Size(50,50), 10);
        ...
        calcOpticalFlowPyrLK(im_gray, im_prev, points_tracked, points_back, status_back, err_back, cv::Size(50,50), 10);
